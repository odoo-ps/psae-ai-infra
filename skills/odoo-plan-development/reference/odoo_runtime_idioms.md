...
The defects this file prevents share one trait: **they pass static lint, install, and the generic smoke, and only surface when a human drives the live UI.** Valid XML + a clean `-i` + a passing `_smoke_module.py` does **not** mean the interaction is correct. Consult this list whenever the addon ships a wizard, an editable list, an inline action button, a status indicator, or a guard on a committed value.
